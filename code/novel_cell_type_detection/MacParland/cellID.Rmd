...
for (fold in seq(1, 5)) {
  ...
  pred_score <- apply(test_data_X_group_gs, 2, max)#apply(test_data_X_group_gs, 2, max)/max(apply(test_data_X_group_gs, 2, max))
  test_data_X_group_gs_prediction_signif <- ifelse(pred_score>2, yes = test_data_X_group_gs_prediction, "Novel")
  ...
  # For detecting novel cell type
  pred_score <- apply(test_data_X_cell_gs, 2, max)#apply(test_data_X_cell_gs, 2, max)/max(apply(test_data_X_cell_gs, 2, max))
  test_data_X_cell_gs_prediction_signif <- ifelse(pred_score>2, yes = as.vector(test_data_X_cell_gs_prediction), "Novel")
  
  # Store prediction, true values, and fold
  final_dataframe_temp_group <- data.frame(pred = test_data_X_group_gs_prediction_signif, true_label = test_data_X$cell_type, fold = fold)
  final_dataframe_temp_cell <- data.frame(pred = test_data_X_cell_gs_prediction_signif, true_label = test_data_X$cell_type, fold = fold)
