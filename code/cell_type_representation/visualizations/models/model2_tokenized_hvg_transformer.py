...
def get_weight(att_mat):
    ...
    #aug_att_mat = att_mat.to(device)
    #print(aug_att_mat.size())
    # Recursively multiply the weight matrices
    joint_attentions = torch.zeros(aug_att_mat.size()).to(device)
    joint_attentions[0] = aug_att_mat[0]
    
    if aug_att_mat.size(0) > 1:
...
class HVGTransformer(nn.Module):
    ...
    def forward(self, x, gene2vec_emb, return_attention=False):
        ...
        if return_attention:
            attn_matrix = torch.mean(attn_matrix[0], dim=1)[:,0,1:]#[:,2,1:]#[:,0,1:]
            #attn_matrix = attn_matrix[0][:,3,:,:]
            #attn_matrix = get_weight(attn_matrix)
            #attn_matrix = torch.mean(attn_matrix, axis=1)
            #attn_matrix = x[:,1:]
