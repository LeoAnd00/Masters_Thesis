...
class VisualizeEnv():
    ...
    def VisualizeCellTypeCorrelations(self, image_path: str=None):
        ...
        colorbar.set_label('Euclidean Distance', fontsize=18) 
        plt.xticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), rotation=60, ha='right', fontsize=14)  # Adjust rotation and alignment
        plt.yticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), fontsize=14)
        plt.title('Normalized euclidean distance between cell type centorids in PCA latent space', fontsize=18)
        plt.tight_layout()  # Adjust layout for better spacing
        # Save the plot as an SVG file
        if image_path:
        ...
        colorbar = plt.colorbar(heatmap, label='Euclidean Distance')
        colorbar.set_label('Euclidean Distance', fontsize=18) 
        plt.xticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), rotation=60, ha='right', fontsize=14)  # Adjust rotation and alignment
        plt.yticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), fontsize=14)
        plt.title('Euclidean distance between cell type centorids in PCA latent space', fontsize=18)
        plt.tight_layout()  # Adjust layout for better spacing
        plt.show()
        ...
        colorbar = plt.colorbar(heatmap, label='Std of Euclidean Distance')
        colorbar.set_label('Euclidean Distance', fontsize=18) 
        plt.xticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), rotation=60, ha='right', fontsize=14)  # Adjust rotation and alignment
        plt.yticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), fontsize=14)
        plt.title('Std of euclidean distance between cell type centorids in PCA latent space', fontsize=18)
        plt.tight_layout()  # Adjust layout for better spacing
        plt.show()
        ...
        plt.xticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), rotation=60, ha='right', fontsize=14)  # Adjust rotation and alignment
        plt.yticks(range(len(self.pred_adata.obs['cell_type'].unique())), self.pred_adata.obs['cell_type'].unique(), fontsize=14)
        plt.title('CV of euclidean distance between cell type centorids in PCA latent space', fontsize=18)
        plt.tight_layout()  # Adjust layout for better spacing
        # Save the plot as an SVG file
        if image_path:
        ...
        for row in range(CV_df.size(0)):
            ...
            if len(non_zero_indices) > 0:
                CV_df2[self.pred_adata.obs['cell_type'].unique()[row]] = CV_df[row, non_zero_indices].flatten().numpy()
