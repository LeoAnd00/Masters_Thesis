...
from functions import train3 as trainer
from models import model3_encoder as model_encoder
...
class benchmark():
    ...
    def in_house_model_tokenized_HVG_transformer(self, save_path: str, umap_plot: bool=True, train: bool=True, save_figure: bool=False):
        ...
        predictions = train_env.predict(data_=adata_in_house, model=model, model_path=save_path, return_attention=True)
        
        adata_attn_latent = sc.AnnData(X=predictions)

        # Apply PCA to the latent space
        sc.pp.pca(adata_attn_latent, svd_solver="auto", n_comps=100)

        adata_in_house.obsm["In_house"] = adata_attn_latent.obsm["X_pca"]#predictions
