...
import torch
import random
from benchmarks.benchmark_generalizability_with_validation2 import benchmark as benchmark
...
# Then:
# sbatch jobscript_generalizability_with_validation_bone_marrow2.sh

def main(data_path: str, model_path: str, result_csv_path: str, pathway_path: str, gene2vec_path: str):
    """
    Execute the generalizability benchmark pipeline when using 20% of training data for validation
    instead of the entire training data set for validation. Selects 20% of data for testing and uses the
        ...
        for seed in r_seeds:
            ...
            while True:  # Keep trying new seeds until no error occurs
                try:
                    ...
                    benchmark_env.in_house_model_encoder(save_path=f'{model_path}Encoder/Generalizability_with_validation2/train_num_{train_num}_seed_{seed}_', train=True, umap_plot=False, save_figure=False)
                    #benchmark_env.in_house_model_tokenized_HVG_transformer(save_path=f'{model_path}Tokenized_HVG_Transformer/Generalizability_with_validation2/train_num_{train_num}_seed_{seed}_', train=True, umap_plot=False, save_figure=False)
                    ...
                    while True:
                        new_seed = random_numbers[counter2]
