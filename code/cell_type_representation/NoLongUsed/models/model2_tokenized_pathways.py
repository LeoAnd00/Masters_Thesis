...
class PathwayTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.cls_token = nn.Parameter(torch.zeros(1, 1, pathway_embedding_dim))
    ...
    def forward(self, pathways, return_attention=False):
        """
        Forward pass of the Pathway Transformer model.
        ...
        pathways = self.feature_embed(pathways)


        cls_token = self.cls_token.expand(pathways.shape[0], -1, -1)
        pathways = torch.cat((cls_token, pathways), dim=1) 
...
class OutputEncoder(nn.Module):
    def __init__(self, 
        ...
        self.norm_layer2 = norm_layer(int(input_dim/4))#norm_layer(int(input_dim/4 + num_pathways))
        self.dropout2 = nn.Dropout(drop_out)
        self.linear2_act = act_layer()
        #self.dropout3 = nn.Dropout(drop_out)
        #self.linear3 = nn.Linear(int(input_dim/4 + num_pathways), int((input_dim/4 + num_pathways)/2)) # num_pathways
        #self.norm_layer3 = norm_layer(int((input_dim/4 + num_pathways)/2))
        #self.linear3_act = act_layer()
        self.output = nn.Linear(int(input_dim/4), output_dim)#nn.Linear(int((input_dim/4 + num_pathways)/2), output_dim)
    ...
    def forward(self, x, pathways):
        ...
        x = self.dropout2(x)
        x = self.linear2(x)
        #x = torch.cat((x, pathways), dim=1) #New
        #x += pathways
        x = self.norm_layer2(x)
        x = self.linear2_act(x)
        #x = self.dropout3(x) # New
        #x = self.linear3(x) # New
        #x = self.norm_layer3(x) # New
        #x = self.linear3_act(x) # New
        x = self.output(x)
        return x
...
class CellType2VecModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.pathway_transformer = PathwayTransformer(mask=mask,
                                                        ...
                                                        mlp_ratio=4, 
                                                        attn_bias=attn_bias,
                                                        drop_ratio=0.5, # Changed from 0.2 to 0.5
                                                        attn_drop_out=0.5, # Changed from 0. to 0.5
                                                        depth=3,
                                                        act_layer=nn.ReLU,
                                                        norm_layer=nn.LayerNorm)
    ...
    def forward(self, x, x_not_tokenized, gene2vec_emb, return_attention=False):
        ...
        if return_attention:
            pathways, attn_matrix = self.pathway_transformer(x_not_tokenized, return_attention) 
        else:
            pathways = self.pathway_transformer(x_not_tokenized, return_attention) 
