...
#
# Model using transformer for HVGs and pathway information, followed by a encoder
#
...
class MultiheadAttention_HVGs(nn.Module):
    """
...
class AttentionMlp_HVGs(nn.Module):
    def __init__(self, 
                 in_features: int, 
                 hidden_features: int, 
                 out_features: int, 
...
class AttentionBlock_HVGs(nn.Module):
    """
    ...
    def __init__(self,
        ...
        super(AttentionBlock_HVGs, self).__init__()

        self.attnblock_norm1 = norm_layer(attn_input_dim)
        self.attnblock_norm2 = norm_layer(attn_input_dim)

        self.attnblock_attn = MultiheadAttention_HVGs(attn_input_dim, 
                                                 attn_embed_dim, 
                                                 num_heads, 
                                                 output_dim, 
                                                 attn_drop_out, 
...
class HVGTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.blocks = nn.ModuleList([AttentionBlock_HVGs(attn_input_dim=HVGs, 
...
class CellType2VecModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.HVG_transformer = HVGTransformer(attn_embed_dim=attn_embed_dim,
                                                HVGs=int(input_dim+num_pathways), 
                                                num_heads=num_heads,
                                                mlp_ratio=mlp_ratio, 
                                                attn_bias=attn_bias,
        ...
        self.output_encoder = OutputEncoder(input_dim=int(input_dim+num_pathways), 
