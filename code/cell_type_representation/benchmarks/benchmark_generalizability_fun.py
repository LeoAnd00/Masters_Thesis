...
class benchmark():
    ...
    def Model1_benchmark(self, save_path: str="trained_models/", umap_plot: bool=True, train: bool=True, save_figure: bool=False):
        ...
        self.Model1 = scib.metrics.metrics(
            self.original_test_adata,
            adata_in_house_test,
            "batch", 
            self.label_key,
        ...
        if save_figure:
            sc.tl.umap(adata_in_house_test)
            sc.pl.umap(adata_in_house_test, color=self.label_key, ncols=1, title=self.celltype_title, show=False, save=f"{self.image_path}Model1_cell_type.svg")
    ...
    def Model2_benchmark(self, save_path: str="trained_models/", umap_plot: bool=True, train: bool=True, save_figure: bool=False):
        ...
        self.Model2 = scib.metrics.metrics(
            self.original_test_adata,
            adata_in_house_test,
            "batch", 
            self.label_key,
        ...
        if save_figure:
            sc.tl.umap(adata_in_house_test)
            sc.pl.umap(adata_in_house_test, color=self.label_key, ncols=1, title=self.celltype_title, show=False, save=f"{self.image_path}Model2_cell_type.svg")
    ...
    def Model3_benchmark(self, save_path: str="trained_models/", umap_plot: bool=True, train: bool=True, save_figure: bool=False):
        ...
        self.Model3 = scib.metrics.metrics(
            self.original_test_adata,
            adata_in_house_test,
            "batch", 
            self.label_key,
    ...
    def make_benchamrk_results_dataframe(self, min_max_normalize: bool=False):
        ...
        if self.Model1 is not None:
            calculated_metrics.append(self.Model1)
            calculated_metrics_names.append("Model1")
        if self.Model2 is not None:
            calculated_metrics.append(self.Model2)
