...
class MultiheadAttention(nn.Module):
    ...
    def __init__(self, 
                 ...
                 attn_bias: bool=True):
        super().__init__()
        assert embed_dim % num_heads == 0, "Embedding dimension has to be 0 modulo number of heads specified."
...
class OutputEncoder(nn.Module):
    def __init__(self, 
        ...
        input_dim -= 1
        self.not_tokenized_norm_layer_in = norm_layer(int(input_dim))
        self.not_tokenized_linear1 = nn.Linear(int(input_dim), int(input_dim/2))
        self.not_tokenized_norm_layer1 = norm_layer(int(input_dim/2))
        self.not_tokenized_linear1_act = act_layer()
        self.not_tokenized_linear2 = nn.Linear(int(input_dim/2), int(input_dim/4))
        self.not_tokenized_norm_layer2 = norm_layer(int(input_dim/4))
        self.not_tokenized_dropout2 = nn.Dropout(drop_out)
        self.not_tokenized_linear2_act = act_layer()
        self.not_tokenized_output = nn.Linear(int(input_dim/4), int(output_dim))
        self.not_tokenized_dropout3 = nn.Dropout(drop_out)
    ...
    def forward(self, x, pathways, x_not_tokenized):
        ...
        x_not_tokenized = self.not_tokenized_norm_layer_in(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_linear1(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_norm_layer1(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_linear1_act(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_dropout2(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_linear2(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_norm_layer2(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_linear2_act(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_dropout3(x_not_tokenized)
        x_not_tokenized = self.not_tokenized_output(x_not_tokenized)
