...
class train_module():
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            for data_inputs, data_labels, data_batches, data_not_tokenized in train_loader:
                ...
                if self.data_env.use_gene2vec_emb:
                    ...
                    preds = model(data_inputs_step, data_not_tokenized_step, use_classifier=use_classifier)
    ...
    def predict(self, 
        ...
        if use_classifier:
            data_env = self.data_env_for_classification
        else:
            data_env = self.data_env

        data_ = prep_test_data(data_, data_env)
        ...
        # Define gene2vec_tensor if gene2ve is used
        if data_env.use_gene2vec_emb:
            gene2vec_tensor = data_env.gene2vec_tensor
            if torch.cuda.device_count() > 1:
                for i in range(1, torch.cuda.device_count()):
                    gene2vec_tensor = torch.cat((gene2vec_tensor, data_env.gene2vec_tensor), dim=0)
            gene2vec_tensor = gene2vec_tensor.to(device)
        ...
        if use_classifier:
            ...
            # Set your threshold for binary classification
            threshold = 0.5
            ...
            for row in binary_preds:
                if np.all(row == -1):
                    ...
                    temp = data_env.onehot_label_encoder.inverse_transform(row) # .reshape(-1, 1)
                    original_labels.append(data_env.label_encoder.inverse_transform(temp))
