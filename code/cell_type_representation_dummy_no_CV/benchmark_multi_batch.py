...
from models import cl_dummy_model2 as scRNASeq_model
...
class benchmark():
    ...
    def __init__(self, 
                 data_path: str, 
                 batch_key: str=["patientID","tissue"], 
        ...
        # Concatenate batch effect metadata in order for models to account for multiple batch effects
        adata.obs['batch'] = ""
        for key in batch_key:
            temp = adata.obs[key]
            adata.obs['batch'] += temp
    ...
    def tosica(self, umap_plot: bool=True):
        ...
        TOSICA.train(adata_tosica, gmt_path='human_gobp', label_name='cell_type',epochs=3,project='hGOBP_TOSICA')
    ...
    def in_house_model(self, save_path: str, umap_plot: bool=True):
        ...
        predictions = train_env.predict(data_=adata_in_house)
