...
class prep_data(data.Dataset):
    ...
    def __init__(self, adata, json_file_path: str, num_pathways: int, HVG: bool, Scaled: bool, HVGs:int = 4000, target_key: str="cell_type", batch_keys: list=None):
        self.adata = adata
    ...
    def __getitem__(self, idx):
        ...
        data_pathways = self.X[idx] * self.pathway_mask # torch.Size([500, 4000])
...
class CellType2VecModel(nn.Module):
    ...
    def __init__(self, 
        ...
        self.blocks = nn.ModuleList([AttentionBlock(attn_input_dim=pathway_emb_dim, 
                                   ...
                                   normlayer_input_dim=pathway_input_dim,
                                   num_heads=num_heads, 
                                   output_dim=pathway_emb_dim,
                                   mlp_ratio=mlp_ratio, 
                                   attn_bias=attn_bias,
                                   mlp_drop=drop_ratio, 
        ...
        self.block_out = AttentionBlock(attn_input_dim=pathway_emb_dim, 
                                   attn_embed_dim=attn_embed_dim,
                                   normlayer_input_dim=pathway_input_dim,
                                   num_heads=num_heads, 
                                   output_dim=1,
        ...
        input_dim = input_dim + pathway_input_dim
        ...
        self.output = nn.Linear(pathway_input_dim, output_dim)
...
class train_module():
    ...
    def train(self, 
        ...
        - pathway_emb_dim (int): Dimension of the pathways embedding before going into the transformer.
        ...
        model = CellType2VecModel(input_dim=self.data_env.X.shape[1],
                                    ...
                                    proj_drop_out=proj_drop_out,
                                    HVGs=self.HVGs,
                                    pathway_input_dim=self.num_pathways,
                                    pathway_emb_dim=pathway_emb_dim)
    ...
    def predict(self, data_, out_path: str, batch_size: int=32, device: str=None):
        ...
        with torch.no_grad():
            if self.batch_keys is not None:
                for data_inputs, _, _, data_pathways in data_loader:
                ...
                for data_inputs, _, data_pathways in data_loader:
...
class prep_test_data(data.Dataset):
    ...
    def __init__(self, adata, perp_data_env, HVG: bool, Scaled: bool, HVGs:int = 4000, target_key: str="cell_type", batch_keys: list=None):
        ...
        # Pathway information
        self.pathway_mask = perp_data_env.pathway_mask
