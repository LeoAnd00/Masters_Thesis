...
class SNNLoss(nn.Module):
    ...
    def forward(self, input, targets, batches=None):
        ...
        if batches is not None:
            ...
            ### Loss contribution to make centorid of each batch effect cluster to be close to the cell type cluster centroid
            # Calcualte centroids of each label cluster:
            target_centroids = {}
            for target in targets.unique():
                # Find the indices of vectors with the current label
                indices = torch.where(targets == target)[0]

                # Extract the vectors with the current label
                vectors_for_label = input[indices]

                # Calculate the centroid for the label cluster
                centroid = torch.mean(vectors_for_label, axis=0)

                # Store the centroid in the dictionary
                target_centroids[target.item()] = centroid.detach() 

            # Calculate centroids of each batch cluster in each label and take the Euclidean distance
            #label_batch_centroids = {}
            loss_centroid_batches = []
            for target in targets.unique():
                target_loss = []
                for batch_idx, batch in enumerate(batches):
                    for batch_key in batch.unique():
                        # Find the indices of vectors with the current label and batch key
                        indices = (targets == target) & (batch == batch_key)

                        if indices.any():

                            # Extract the vectors with the current label and batch key
                            vectors_for_label_batch = input[indices]

                            # Calculate the centroid for the label and batch cluster
                            centroid = torch.mean(vectors_for_label_batch, axis=0)

                            # Calculate loss
                            temp_loss = torch.norm(target_centroids[target.item()] - centroid, p=2)/centroid.size(0)#nn.MSELoss()(target_centroids[target.item()],centroid)
                            target_loss.append(temp_loss)

                loss_centroid_batches.append(torch.mean(torch.stack(target_loss)))

            loss_centroid_batches = torch.mean(torch.stack(loss_centroid_batches, dim=0))

            ### Loss contribution to enforce data of same batch to be further away for each other
            loss_batches = []
            ...
            loss = 0.9*loss_target + 0.05*loss_centroid_batches + 0.05*loss_batch
...
class train_module():
    ...
    def train(self, 
        ...
        cv_loss, preds = self.train_model(model=model, 
                                    optimizer=optimizer, 
        ...
        print()
        print(f"Loss score: {np.mean(cv_losses):.4f}")
        print()
    ...
    def predict(self, data_, batch_size: int=32, device: str=None):
        ...
        for model in self.models:
            ...
            with torch.no_grad():
                if self.batch_keys is not None:
                    for data_inputs, data_labels, data_batches in data_loader:

                        data_inputs = data_inputs.to(device)
                        #data_labels = data_labels.to(device)
                        #data_batches = data_batches.to(device)
